...
def get_ingre_prob_from_model(uploaded_image):
    ...
    normalized_tensor = (normalized_tensor - min_normalized_value) / (max_normalized_value - min_normalized_value)
    # 0-1
...
def page_1(username):
    ...
    if st.session_state.stage >= 1:
        ...
        # if 'probability_scores' not in st.session_state:
            ...
            st.session_state.pos_probability = get_pos_probability(st.session_state.probability_scores)
        # if 'normalized_matrix' not in st.session_state:
            normalized_matrix = get_normalized_co_occurrence_matrix()
            st.session_state.normalized_matrix = np.where(normalized_matrix < threshold/100, 0, normalized_matrix)
        ...
        if st.session_state.init_prediction == False:
            if st.session_state.predict_method == "method_1":
                st.session_state.predict_ingres = get_current_candidate_method1(candidate_nums, st.session_state.probability_scores, st.session_state.mask, st.session_state.normalized_matrix)
            if st.session_state.predict_method == "method_2":
                ...
                st.session_state.predict_ingres = get_current_candidate(candidate_nums, st.session_state.pos_probability, st.session_state.mask, st.session_state.normalized_matrix)
    ...
    if st.session_state.stage >= 2:
        ...
        for item in st.session_state.predict_ingres:
            st.session_state.selected_options[item] = c2.checkbox(ingres[int(item)]) # key=f'ingre_{item}')
        ...
        options = c2.multiselect(
            ...
            ingres_without_NA,
            [])
        st.session_state.selected_ingres = [item for item, selected in st.session_state.selected_options.items() if selected]
        ...
        if st.session_state.predict_method == "method_1":
            st.session_state.mask = update_mask_method1(st.session_state.selected_ingres, st.session_state.mask)
        if st.session_state.predict_method == "method_2":
            st.session_state.mask = update_mask(st.session_state.selected_ingres, st.session_state.mask, st.session_state.normalized_matrix)

        if c2.button('新しい食材候補を生成する'):
            st.session_state.click_dict["button"] += 1
            st.session_state.mask = update_mask_method1(st.session_state.predict_ingres, st.session_state.mask) # delete other unselected ingredients
        ...
        if st.session_state.ingre_finish == True:
            ...
            st.session_state.click_dict["checkbox"] = len(st.session_state.selected_ingres) - st.session_state.click_dict["input_text"]
            st.session_state.selected_ingres = [item for item, selected in st.session_state.selected_options.items() if selected]
            if st.session_state.saved == False:
                save_results(st.session_state.username, image, st.session_state.predict_method, st.session_state.selected_ingres, ingres, st.session_state.click_dict)
...
############
usernames = ['l_wang', 'yamakata', 'admin', 'f001', 'f002', 'f003', 'f004', 'f005', 'f006', 'f007', 'f008', 'f009', 'f010', 'u001', 'u002', 'u003', 'u004','u005', 'u006','u007', 'u008','u009', 'u010','u011', 'u012','u013', 'u014','u015', 'u016','u017', 'u018','u019', 'u020', 'u021', 'u022', 'u023', 'u024', 'u025', 'u026', 'u027', 'u028', 'u029', 'u030', 'u031', 'u032', 'u033', 'u034', 'u035', 'u036', 'u037', 'u038', 'u039', 'u040', 'u041', 'u042', 'u043', 'u044', 'u045', 'u046', 'u047', 'u048', 'u049', 'u050']
passwords = ['12345', '54321', 'ad1234', 'f001', 'f002', 'f003', 'f004', 'f005', 'f006', 'f007', 'f008', 'f009', 'f010', '45260', '75340', '44107', '62902', '23960', '75281', '99990', '86609', '76326', '43646', '72447', '75362', '07098', '75855', '26734', '24302', '03279', '86585', '15465', '45127', '64148', '33167', '46420', '32709', '47293', '06965', '39102', '58952', '54210', '95708', '00531', '75240', '66479', '34432', '42353', '35340', '48337', '71182', '39435', '07803', '24257', '86121', '97796', '86071', '90097', '12565', '56250', '98969', '31541', '80025']
