...
def save_results(username, image_file, method, ingredients, ingres_convert, click_dict):
    ...
    output_path = f"{directory}result_{next_serial_number}_{method}.json"
...
def page_1(username):
    ...
    if uploaded_image is not None and st.session_state.stage >= 1:
        ...
        c1.write(st.session_state.now_page)
        ...
        if st.session_state.stage == 1:
            if st.session_state.predict_method == "method_1":
                predictions = get_current_candidate_method1(
                    candidate_nums,
                    probability_scores,
        ...
        if st.session_state.stage >= 2:
            ...
            options = c2.multiselect("リストにない食材を検索:", ingres, [])
            selected_ingres = [
            ...
            if st.session_state.predict_method == "method_1":
                st.session_state.mask = update_mask_method1(
                    selected_ingres, st.session_state.mask
                )
            if st.session_state.predict_method == "method_2":
            ...
            if c3.button("完了", on_click=set_state, args=[3]):
                ...
                c3.success("回答を記録しました！次のページに進めてください。")
                # if c3.button('次の方法へ'):
                # st.session_state.now_page = 2
                # main(username)
                # page_2(username)
                # url = 'http://163.220.177.123/l_wang/DAI/test'
                # c3.link_button("次の方法へ", url)
...
def main(username):
    ...
    if st.session_state.now_page == 1:
        page_1(username)
    # elif st.session_state.now_page == 2:
    #     page_2(username)
